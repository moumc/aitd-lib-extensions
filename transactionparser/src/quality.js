...
the quotient of TakerPays/TakerGets. It uses drops (1e-6 AITD) for AITD values.
*/

function adjustQualityForAITD(quality, takerGetsCurrency, takerPaysCurrency) {
  var numeratorShift = (takerPaysCurrency === 'AITD' ? -6 : 0)
  var denominatorShift = (takerGetsCurrency === 'AITD' ? -6 : 0)
...
function parseQuality(qualityHex, takerGetsCurrency, takerPaysCurrency) {
  ...
  return adjustQualityForAITD(quality, takerGetsCurrency, takerPaysCurrency)
}
