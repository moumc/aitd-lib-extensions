...
var lsfSell = 0x00020000 // see "lsfSell" flag in aitdd source code
...
function aitdToUnixTimestamp(rpepoch) {
...
function getExpirationTime(node) {
  ...
  return (new Date(aitdToUnixTimestamp(expirationTime))).toISOString()
}
...
function getQuality(node) {
  ...
  var takerGetsCurrency = takerGets.currency || 'AITD'
  var takerPaysCurrency = takerPays.currency || 'AITD'
 ...
 * Returns changes grouped by Aitd account.
 *
 *  @param {Object} metadata - Transaction metadata as return by aitd-lib
 *  @returns {Object} - Orderbook changes grouped by Aitd account
 *
 */
